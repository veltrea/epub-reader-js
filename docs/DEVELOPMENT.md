...
Runs seven checks together; CI runs the same set. **[CHECKS.md](CHECKS.md) covers
each one: what it looks at, and what to do when it stops you.** Only checks whose answer is
decided without asking anyone belong here. Anything needing judgement — whether a
design is right, what may be published — stays out: a warning that stops a person
every time is a warning nobody reads.

### What each check asks for when you add something

**A check is not an accusation; it tells you what to do next.** When one stops,
do what it says and move on. Here is what reacts to what.

| What you added | Check that reacts | What to do |
|---|---|---|
| A menu item | Menu wiring | Add the handler to the table in `reader.js` / `shelf.js`. If the item only makes sense on one screen, pass `reader` / `shelf` to `enabled` in `menu.rs` so it greys out |
| Text the user sees | Hard-coded UI strings | Add the key to **both** `src/locales/ja.json` and `en.json`, and call it through `t('key')` |
| A menu item, setting, or test-bus command | Feature inventory | Write the new entries into the manual, then run `node scripts/feature-inventory.mjs --write` |
| A feature worth documenting | Symbol matching | Mark the manual passage with `<!-- menu:xxx -->`. A feature with no marked passage stops the run |
| A quoted label in a document | Manual labels | Make the quote match what is in `ja.json`. If the app's own label is the wrong one, fix that first |
| Pure logic | Logic unit tests | Add a test to `tests/logic.test.mjs`. Keeping the logic DOM-free is what makes that possible |
| Version or packaging config | Config and version | Keep the two version numbers in step; keep private paths untracked |
...
To take something out of a check's net, add it to that check's skip list **with the
reason written beside it**. A skip with no reason cannot be removed by the next
person.
...
`ttsHighlightInfo` `ttsHighlightRects` (the read-aloud band's range and rects; use them to
check numerically that ruby lines do not widen the band)
`dictList` `dictAdd` `dictUpdate` `dictDelete` `dictGet` `dictSet` `setRules` (reading
dictionary; pass `scope`: `common` = all books (default) / `book` = this book only /
`merged` = the same order read-aloud uses)
`openDictForm` `openDictList` `dictOpenEntry` `dictSetScope` `dictBack` `dictDone` `dictClose`
(drive the dictionary *screen* the way a person would, to check the screen really works and not
just the stored data. **Registering and browsing are separate screens, so there are two ways to
open one:** `openDictForm` for the registration screen, `openDictList` for the list.
`dictOpenEntry` opens a row of the list, `dictSetScope` moves an entry to the other half,
`dictBack` goes to the list, `dictDone` saves and closes the registration screen, and
`dictClose` closes the list. All of them exist on the shelf too. Pass `place` as
`{section, offset}` to `openDictForm` to get the "reading for this spot only" form.
**The closing commands return only once the save has finished** — send the next write without
waiting and the save still running will overwrite it with stale content.)
`applyRules` and `dictPrepare` also take `place`, so spot-tied entries can be checked without
reading aloud. `ttsHighlightInfo` returns `place` — where in the book the sentence being read
sits — to compare against a registered spot.
`ttsPlay/Pause/Resume/Stop/State` `autoPagerStart/Stop` (reader) · `screenshot`
`navigate` `click` `domInfo` `setValue` (both).

`setValue` writes into an input and fires `input` and `change`. `click` only calls the
element's `click()`, which does not move typing focus, so this is how text gets in from
outside. (It used to be done by moving the real mouse and pasting, aiming at a fraction of
the window — which missed as soon as the screen's contents shifted.)
