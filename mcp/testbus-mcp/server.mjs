...
// 待機中の応答が、まだ本当に生きているか。
// ページを読み込み直すと、前のページが出していた /pull の接続はそこで切れる。
// 切れた応答へ書き込んでも Node は例外を出さず、黙って捨てる。つまり
// **配信したつもりでコマンドが消える**。生きているものだけに配る。
function alive(w) { return !w.res.writableEnded && !w.res.destroyed && !w.req.destroyed; }

// 切れた待機を取り除く。close を受け取り損ねた場合の受け皿でもある。
function dropDeadWaiters() {
  for (let i = pullWaiters.length - 1; i >= 0; i--) {
    const w = pullWaiters[i];
    if (alive(w)) continue;
...
function deliverIfPossible() {
  ...
  dropDeadWaiters();
  while (jobs.length) {
    const idx = pullWaiters.findIndex((w) => w.kind === 'app' && alive(w));
    if (idx < 0) break;
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url.startsWith('/pull')) {
    ...
    const waiter = { req, res, timer: null, kind };
    waiter.timer = setTimeout(() => {
      const i = pullWaiters.indexOf(waiter);
      if (i >= 0) pullWaiters.splice(i, 1);
      res.writeHead(204, cors); res.end();
    }, 25000);
    // 接続が切れたら**その場で**待機の列から外す。外さないと、次のコマンドが
    // この死んだ接続へ配られて消える(ページを読み込み直した直後に起きていた)。
    res.on('close', () => {
      const i = pullWaiters.indexOf(waiter);
      if (i < 0) return;
  ...
  if (req.method === 'GET' && req.url === '/health') {
    ...
    // waiters は切り分け用。繋がらないときに「待機が 0 なのか、死んだ待機が居るのか」を
    // 外から見分けられるようにしてある。
    return res.end(JSON.stringify({
      ok: true,
      appConnected: appWaiting || (Date.now() - lastPullAt < 5000),
      queued: jobs.length,
      waiters: pullWaiters.map((w) => w.kind || '(none)'),
    }));
  }
