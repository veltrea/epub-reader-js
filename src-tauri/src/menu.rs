...
/// 環境変数(LANG 等)→ OS ごとの設定、の順で見る。
/// .app を Finder から起動すると環境変数が空なので、macOS では `defaults` が実質の判定元。
/// **Windows には LANG が無い**ので、環境変数はまず当てはまらず、Win32 の呼び出しが判定元になる。
fn os_lang() -> String {
...
/// Windows の表示言語から ja / en を決める。
///
/// **外部のコマンドを起動しない。** `powershell -Command "(Get-Culture).Name"` でも取れるが、
/// 起動に 165 ミリ秒以上かかる（Windows 機で実測）。ここはアプリの起動のたびに通る場所なので、
/// その分だけ起動が遅くなる。`windows-sys` は Tauri が既に使っているので、
/// 新しくコンパイルするものは増えない。
#[cfg(target_os = "windows")]
fn win_ui_lang() -> String {
    langid_to_lang(unsafe { windows_sys::Win32::Globalization::GetUserDefaultUILanguage() })
}

/// LANGID（Windows が言語を表す 16 ビットの数）から ja / en を決める。
///
/// **OS を呼ばないので、Windows でなくても試験できる。** `win_ui_lang` と分けてあるのは
/// そのため。下位 10 ビットが主言語を表し、日本語は 0x11 である。上位 6 ビットは地域
/// （日本語なら日本・その他）を表すが、ja と en の 2 択には要らない。
#[cfg(any(target_os = "windows", test))]
fn langid_to_lang(langid: u16) -> String {
...
pub fn build<R: Runtime>(
    ...
    // **アプリ名のメニューは macOS の作法である。Windows には無い。**
    // 中身の置き場所も OS ごとに違うので、そこに合わせる（2026-09-03 に実機で確かめた）:
    //
    //   macOS  : アプリ名のメニューに「〜について」「設定…」「終了」をまとめる。
    //            「サービス」「隠す」系も、この OS の機能なのでここに並べる。
    //   Windows: アプリ名のメニューを**作らない**。
    //            「設定…」と「終了」は「ファイル」の末尾、「〜について」は「ヘルプ」へ置く。
    //            「サービス」「隠す」系は Windows に同じ機能が無いので**出さない**。
    #[cfg(target_os = "macos")]
    ...
    #[allow(unused_mut)]
    let mut fb = SubmenuBuilder::new(app, s.t("menu.file"))
        .item(&new_profile)
        ...
        .separator()
        .item(&export_menu);
    // Windows の作法: 「設定…」と「終了」は「ファイル」の末尾に置く。
    // macOS ではアプリ名のメニューに入っているので、ここには足さない。
    ...
    // 登録する画面と、登録済みを見る画面は別の項目にする。一語だけ登録したい人に
    // 長い一覧を通らせない。
    let tts_dict_add = item("tts.dictAdd", "menu.file.dictAdd", None, true)?;
    ...
    // **メニューの並びも OS の作法に合わせる。**
    //   macOS  : アプリ名 / ファイル / 編集 / 表示 / 移動 / 読み上げ / ウインドウ
    //   Windows: ファイル / 編集 / 表示 / 移動 / 読み上げ / ヘルプ
    //            「ウインドウ」は macOS の作法で、窓が 1 つのアプリの Windows 版には置かない。
    //            代わりに「ヘルプ」を置き、「〜について」をそこへ入れる。
    #[cfg(target_os = "macos")]
    {
        MenuBuilder::new(app)
    ...
    #[cfg(not(target_os = "macos"))]
    {
        let _ = &window_menu; // macOS 以外では使わない
        let help_menu = SubmenuBuilder::new(app, s.t("menu.help"))
            .about_with_text(s.t("menu.app.about"), None)
            .build()?;
        MenuBuilder::new(app)
            .items(&[&file_menu, &edit_menu, &view_menu, &go_menu, &tts_menu, &help_menu])
            .build()
    }
}
...
mod tests {
    ...
    // Windows の表示言語の判定。OS を呼ばない部分だけを見るので、Mac でもこの試験は走る。
    #[test]
    fn langid_picks_ja_only_for_japanese() {
        assert_eq!(langid_to_lang(0x0411), "ja"); // 日本語（日本）
        assert_eq!(langid_to_lang(0x0409), "en"); // 英語（米国）
        assert_eq!(langid_to_lang(0x0809), "en"); // 英語（英国）
        assert_eq!(langid_to_lang(0x0804), "en"); // 中国語（簡体字・中国）
        assert_eq!(langid_to_lang(0x0412), "en"); // 韓国語（韓国）
    }
