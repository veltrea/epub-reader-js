...
/// キーを 1 つ消す。無ければ何もしない(消す対象が既に無いのは失敗ではない)。
#[tauri::command]
fn store_delete(name: String) -> Result<(), String> {
    let p = store_path(&name);
...
/// 機械をスリープさせる。
///
/// macOS: `pmset sleepnow`。管理者権限も追加の許可も要らない。
/// Windows: `rundll32 powrprof.dll,SetSuspendState`。こちらも追加の許可は要らない。
#[tauri::command]
fn system_sleep() -> Result<(), String> {
    ...
    {
        // 引数の 3 つは順に「休止するか・強制するか・起こす合図を無効にするか」。
        // 先頭を 0 にしてスリープを頼む。**ただし休止(hibernate)が有効な機械では、
        // 0 を渡しても Windows が休止を選ぶことがある。** これは Windows の仕様で、
        // こちらから止められない。どちらでも「画面が消えて止まる」ので実害は無い。
        std::process::Command::new(system32("rundll32.exe"))
    ...
    Err("sleep is only implemented on macOS and Windows".to_string())
}

/// Windows の System32 の中にある実行ファイルの、完全なパスを組み立てる。
///
/// **名前だけで呼ばない。** PATH の先頭に同じ名前のものが置かれていると、
/// そちらが実行されてしまう。macOS 側が `/usr/bin/pmset` と完全なパスで書いているのと同じ理由。
#[cfg(target_os = "windows")]
...
/// 機械をシャットダウンする。
///
/// macOS: `shutdown -h now` は root が要るが、System Events 経由ならログイン中のユーザー権限で
/// 正規の終了処理(各アプリへの終了問い合わせ)が走る。代わりに**初回だけ**
/// 「"System Events" を操作する許可」の OS ダイアログが出る。
/// Windows: `shutdown /s /t 0`。ログイン中のユーザーの権限でそのまま実行できる。
#[tauri::command]
fn system_shutdown() -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        // /s = シャットダウン、/t 0 = 待ち時間なし。
        // /f(強制)は付けない。付けると保存していない書類が問い合わせ無しで捨てられる。
    ...
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    Err("shutdown is only implemented on macOS and Windows".to_string())
}

/// 読み上げエンジンのアプリを起動する。
///
/// **起動できるのは既知の 2 つだけ**にしてある。アプリ名をそのまま受け取る作りにすると、
/// テストバスや細工したページから好きなプログラムを起動させられてしまう。
///
/// `open -a` は既に動いていれば何もしない(前面に出るだけ)ので、二重起動にならない。
#[tauri::command]
fn launch_tts_engine(engine: String) -> Result<(), String> {
    let app = match engine.as_str() {
        "voicevox" => "VOICEVOX",
        "aivis" => "AivisSpeech",
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if status.success() {
            ...
            Err(format!("could not launch {app}"))
        }
    }
    #[cfg(target_os = "windows")]
    {
        // **入っていないことのほうが多い。** そのときは起動しようとせず、見つからないと返す。
        // フロントはこの返事を見て「エンジンが見つかりません」と案内する。
        let exe = find_engine_exe(app).ok_or_else(|| format!("could not find {app}"))?;
    ...
    {
        let _ = app;
        Err("launching the speech engine is only implemented on macOS and Windows".to_string())
    }
}

/// 読み上げエンジンの実行ファイルを探す。見つからなければ None。
#[cfg(target_os = "windows")]
fn find_engine_exe(name: &str) -> Option<std::path::PathBuf> {
    engine_exe_candidates(name, &engine_search_roots())
        .into_iter()
        .find(|p| p.is_file())
}
...
/// エンジンが入っていそうな場所を並べる。
///
/// VOICEVOX も AivisSpeech も、既定では `%LOCALAPPDATA%\Programs\<名前>\` に入る。
/// ただしインストールのときに変えられるので、`Program Files` の側も見る。
/// **どこにも無ければ諦める。** レジストリまで調べに行くと、名前の付け方が版で変わったときに
/// 静かに動かなくなる。見つからないと正直に返すほうが、あとで直しやすい。
#[cfg(target_os = "windows")]
fn engine_search_roots() -> Vec<std::path::PathBuf> {
    let mut roots = Vec::new();
    if let Ok(v) = std::env::var("LOCALAPPDATA") {
        roots.push(std::path::PathBuf::from(v).join("Programs"));
    }
...
/// 探す場所の一覧と名前から、試すパスの並びを作る。
///
/// **OS を呼ばないので、Windows でなくても試験できる。** インストーラが作るのは
/// 「場所 / 名前 / 名前.exe」という形（例: `...\Programs\VOICEVOX\VOICEVOX.exe`）。
#[cfg(any(target_os = "windows", test))]
fn engine_exe_candidates(name: &str, roots: &[std::path::PathBuf]) -> Vec<std::path::PathBuf> {
    roots
        .iter()
        .map(|r| r.join(name).join(format!("{name}.exe")))
        .collect()
}
...
/// xcap から見えているウィンドウを全部並べる。
///
/// **`capture_window` が「ウィンドウが見つかりません」と言うとき、何が見えているのかを
/// 確かめるために使う。** Windows でその症状が出ており（2026-08-30）、
/// 題名が原因ではないところまでは分かっている。
/// テストバスと同じ理由で、配布版では無効。
#[tauri::command]
fn list_windows() -> Result<Vec<serde_json::Value>, String> {
    if !testbus_allowed() {
        return Err("list_windows is disabled in release builds \
...
/// コマンドラインの引数から、ファイルのパスらしいものだけを取り出す。
///
/// `-` で始まるものは Tauri や OS への指示なので除く。実在するかどうかと、本かどうかは
/// このあと `expand_book_paths` が見る。
/// **OS を呼ばないので、Windows でなくても試験できる。**
#[cfg(any(target_os = "windows", test))]
...
pub fn run() {
    ...
    // Windows: 関連付けから本を開くたびに新しいプロセスが立つのを防ぐ。
    // 2 つ目のプロセスが受け取った本のパスを 1 つ目へ渡し、そちらで開く。
    // **このプラグインは最初に登録する**（Tauri の作法。あとに回すと効かないことがある）。
    // macOS は同じアプリを二重に起動しないので、この対処は Windows だけで要る。
    #[cfg(target_os = "windows")]
    {
        builder = builder.plugin(tauri_plugin_single_instance::init(|app, argv, _cwd| {
            use tauri::Manager;
            // 既にある窓を前へ出す。最小化されていたら元の大きさに戻す。
            if let Some(w) = app.get_webview_window("main") {
                let _ = w.unminimize();
                let _ = w.set_focus();
            }
            // 引数の見方は起動時（setup）とまったく同じ。
            let paths = expand_book_paths(file_args(argv.into_iter().skip(1)));
            if !paths.is_empty() {
    ...
    builder
        ...
        .setup(|app| {
            ...
            // Windows でファイルの関連付けから開いたとき、本のパスは**コマンドラインの引数**で届く。
            // macOS の `RunEvent::Opened` に当たるものが Windows には無いため、受け取る場所が別になる。
            // macOS 側の挙動は変えないので、ここは Windows だけにしてある。
            #[cfg(target_os = "windows")]
            {
                let paths = expand_book_paths(file_args(std::env::args().skip(1)));
                if !paths.is_empty() {
                    push_pending_open(paths);
                }
            }
            Ok(())
        })
        ...
        //
        // **`RunEvent::Opened` は macOS と iOS にしかない。** Tauri がこの variant 自体を
        // その 2 つでしか作らないので、Windows で素通しにすると
        // 「no variant named `Opened` found for enum `RunEvent`」でコンパイルが失敗する。
        // Windows は起動時のコマンドライン引数でファイルが渡るので、受け取る場所が別になる。
        .run(|app, event| {
            ...
            // macOS 以外ではこのクロージャで何もしない。引数が未使用だという警告だけ消す。
...
mod tests {
    ...
    // Windows で関連付けから開いたときの、引数の選び分け。
    // OS を呼ばない部分だけを見るので、Mac でもこの試験は走る。
    #[test]
    fn file_args_drops_switches_but_keeps_paths() {
        let got = file_args(
            ["--flag", "C:\\books\\a.epub", "-v", "D:\\b.cbz", "--", "e.epub"]
                .map(String::from),
        );
        // `-` で始まるものは全部除く。`--` そのものも除く。
        assert_eq!(got, vec!["C:\\books\\a.epub", "D:\\b.cbz", "e.epub"]);
    }

    // Windows で読み上げエンジンを探す場所の作り方。
    // OS を呼ばない部分だけを見るので、Mac でもこの試験は走る。
    #[test]
    fn engine_paths_follow_the_installer_layout() {
        let roots = vec![
            std::path::PathBuf::from("local").join("Programs"),
            std::path::PathBuf::from("progfiles"),
        ];
        let got = engine_exe_candidates("VOICEVOX", &roots);

        // 探す場所の数だけ候補ができる。
        assert_eq!(got.len(), 2);
        // 「場所 / 名前 / 名前.exe」の形になっている。
        for p in &got {
            assert_eq!(p.file_name().unwrap(), "VOICEVOX.exe");
            assert_eq!(p.parent().unwrap().file_name().unwrap(), "VOICEVOX");
        }
        // 順番は入れ替わらない（%LOCALAPPDATA% を先に見る）。
        assert!(got[0].starts_with("local"));
        assert!(got[1].starts_with("progfiles"));
    }
