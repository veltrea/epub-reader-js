...
export class TTS {
    ...
    /**
     * mark の名前から、その文の range を返す。**ハイライトは動かさない。**
     * ※ foliate-js 本家からのローカル改変。読み上げ辞書の「この場所だけ」の登録を
     *    照合するのに、その文が章の何文字目から始まるかが要る。setMark だと
     *    ハイライトが動いてしまうので、読み取るだけの関数を別に用意してある。
     */
    rangeOf(mark) {
        return this.#ranges?.get(mark) ?? null
    }
    setMark(mark) {
