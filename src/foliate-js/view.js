...
// ここにあった PDF の判定関数は削除した。このアプリは PDF を受け付けない。
// ※ foliate-js 本家からのローカル改変。経緯は CHANGELOG の 0.4.0 を参照。
...
export const makeBook = async file => {
    ...
    // ここにあった PDF の分岐(判定して pdf.js を読み込む 4 行)は削除した。
    // 取り込みの入口で PDF を受け付けないので、この場所へは届かない。
    // ※ foliate-js 本家からのローカル改変。経緯は CHANGELOG の 0.4.0 を参照。
    else {
