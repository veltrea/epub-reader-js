...
export const DEFAULT_SETTINGS = {
  ...
  // ---- 組版を直すときの道具(読者には見せない) ----
  // [feat:measure-grid][reader] 測定オーバーレイ(色の物差し)を読書画面に重ねる
  // 入れている間だけ、読書画面のバーに測定グリッドのボタンが出る。既定は入れない。
  debugMode: false,
};
...
/** 本の id を保存キーに使える形にする(reader.js の safeId と同じ処理)。 */
function bookKey(bookId) { return String(bookId || '').replace(/[^a-zA-Z0-9_-]/g, ''); }

/**
 * **この本だけ**に効く読み上げ辞書。全体の辞書とまったく同じ形の配列。
 *
 * 形を揃えてあるので、編集画面も compile も prepare もそのまま両方に使える。
 * 読み上げに渡すときは `[...全体, ...この本]` の順に並べる。normalizeList が
 * `種類 + 表記` をキーに**後勝ち**で畳むので、同じ表記があればこの本の側が勝つ。
 *
 * 全体の辞書(`dict`)は書棚ごとに分けているが、こちらは分けない。本の id は
 * 取り込みのたびに新しく振られ、書棚をまたいで同じにならないためである。
 * 本別のカスタム CSS(`css-<本のid>`)と同じ考え方。
 */
export async function loadBookDict(bookId) {
  if (!bookId) return [];
  return (await storeGet('dict-' + bookKey(bookId))) || [];
}
export async function saveBookDict(bookId, list) {
  if (!bookId) return;
  await storeSet('dict-' + bookKey(bookId), list);
}
