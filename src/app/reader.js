...
import {
  loadSettings, saveSettings, loadLibrary, loadDict, loadBookDict, saveBookDict,
...
let tts;
let saveTimer = null;
let measureMode = '';       // 測定オーバーレイ(色の物差し)を出しているか: '' | 'window'
...
function applyReaderStyles() {
  ...
  // いま開いている本文にも入れ直す。理由は adoptReaderCSS のコメントにある。
  try {
    for (const c of (view.renderer?.getContents?.() || [])) adoptReaderCSS(c?.doc);
  } catch { /* noop */ }
  applyReaderPageTheme(settings);
}

// 文字サイズ・行間・テーマの指定を、本文へ **adoptedStyleSheets で** 入れる。
//
// **なぜ二重に入れるか（2026-09-03 に Windows 実機で確かめた）:**
// foliate-js の `renderer.setStyles()` は `<style>` 要素の中身を書き換える作りである。
// **Windows(WebView2) は、あとから作った `<style>` 要素を無視する。** macOS(WKWebView)
// では効くので、Windows だけで設定が本文に届かなくなっていた。
// 同じ本文で 3 通りを試したところ、`<style>` だけが効かず、要素への直接指定と
// adoptedStyleSheets は両方の OS で効いた。**だから adoptedStyleSheets にも入れる。**
// `setStyles` の呼び出しは残してある——macOS では今までどおり効いており、
// **`src/foliate-js/` は上流のクローンなので書き換えない**という決まりがあるため。
//
// **入れるのは `readerCSS` の「後ろ」のぶんだけ。** 前のぶんは「本の CSS より弱くして
// おきたい」指定で、adoptedStyleSheets は他のどの指定よりも後に効くので、
// 入れると強さが逆転して本の指定を上書きしてしまう。
//
// 本文ごとに 1 枚のシートを持ち回る。章が変わると本文の document が別物になるので、
// WeakMap で結び付けておく（本文が捨てられればシートも一緒に消える）。
const adoptedByDoc = new WeakMap();
function adoptReaderCSS(doc) {
  if (!doc) return;
  const win = doc.defaultView;
  if (!win?.CSSStyleSheet || !('adoptedStyleSheets' in doc)) return;
  const css = readerCSS(settings);
  const post = Array.isArray(css) ? css[1] : css;
  try {
    let sheet = adoptedByDoc.get(doc);
    if (!sheet) {
      sheet = new win.CSSStyleSheet();
      adoptedByDoc.set(doc, sheet);
      doc.adoptedStyleSheets = [...doc.adoptedStyleSheets, sheet];
    }
    sheet.replaceSync(post || '');
  } catch { /* noop */ }
}
...
async function main() {
  // **一番先に**テストバスを立ち上げる。この後の読み込みが 1 つでも失敗すると main は
  // そこで止まるので、後ろに置くと「画面は出ているのに外からさわれない」状態になる。
  // 登録している関数は、呼ばれたときに初めて中を見る(クロージャ)ので、
  // まだ何も読み込んでいないこの時点で登録して構わない。
...
async function openBook() {
  ...
  tts = new TTSController(view, () => settings, onTTSState, onTTSError, onTTSNotice);
  await refreshDict();
}
...
async function refreshDict() {
  try { compiledDict = compileDict(await mergedDict()); } catch { compiledDict = []; }
  tts?.setDictionary(compiledDict);
  return compiledDict;
}

/** 辞書の画面へ渡す「いま開いている本」。本を開いていなければ null。 */
function dictBook() { return bookId ? { id: bookId, title: $('#book-title')?.textContent || '' } : null; }

/**
 * 読み上げに渡す辞書。**すべての本の辞書 → この本だけの辞書**の順に並べる。
 * normalizeList(compile の中で呼ばれる)が「種類 + 表記」をキーに後勝ちで畳むので、
 * 同じ表記が両方にあれば**この本の側が勝つ**。レイヤーの指定は畳んだあとに
 * 並べ替えられるので、そのまま効く。
 */
async function mergedDict() {
  const [common, book] = await Promise.all([loadDict(), loadBookDict(bookId)]);
  return [...common, ...book];
}

/**
 * 読み辞書を保存して、読み上げに渡す形を作り直す。
 * 辞書を書き換える経路をここ 1 本にまとめてある。保存だけして作り直しを忘れると、
 * 画面と実際に読み上げる内容が食い違う（利用者からは何も起きていないように見える）。
 *
 * scope は 'common'（すべての本。既定）か 'book'（この本だけ）。
 */
async function saveDictList(list, scope = 'common') {
  const { saveDict } = await import('./store.js');
  if (scope === 'book') await saveBookDict(bookId, list);
  else await saveDict(list);
  await refreshDict();
  return list;
}

/** テストバス用: scope の側の辞書を読む。'book' はこの本だけ、それ以外はすべての本。 */
async function loadDictScoped(scope = 'common') {
  return scope === 'book' ? await loadBookDict(bookId) : await loadDict();
}
...
// ---------------------------------------------------------------------------
// 測定オーバーレイ(色の物差し)。組版を直すときの道具で、読者には見せない。
// 設定の「デバッグモード」を入れている間だけ、バーにボタンが 1 つ増える。
// 描き方と符号化は measure.js にある(仕様書 §14.1–14.4)。
// ---------------------------------------------------------------------------

/** いま開いている本文の document を全部返す。 */
function contentDocs() {
  try { return (view?.renderer?.getContents?.() || []).map((c) => c?.doc).filter(Boolean); }
  catch { return []; }
}
 ...
 * 物差しを、いまの状態どおりに描き直す。
 *
 * **物差しは窓に 1 枚だけ敷く。本文の枠(iframe)の中には入れない。**
 * 仕様書 §14.2 は「ネイティブ層」と「WebView 本文」の 2 枚に同じ物差しを敷けと書いている。
 * 元アプリ(Readium)では本文の枠が画面 1 面ぶんの大きさで、その中に敷けば画面に重なった。
 * **foliate は違う。** 本文の枠を「その章のページ全部を並べた大きさ」に広げ、
 * 見せたい面まで動かして 1 面ぶんだけ覗かせる。実測すると、窓の高さが 792 のときに
 * 枠の高さは 2784 あった。この中に `position: fixed` で物差しを敷くと、
 * **物差しの原点が画面の外へ出て、目盛りが読めない。**
 * 表示エンジン(`src/foliate-js/`)は書き換えない決まりなので、この構造は変えられない。
 *
 * **代わりに、測った数の側を窓の座標へそろえた**（`measureOverlay`）。
 * 物差しが 1 枚でも、写真から読んだ色と数を突き合わせられる——それが目的だったので、
 * 目的は満たしている。
 */
function applyMeasureGrid() {
  if (measureMode) measure.showMeasureOverlay(document, window);
  else measure.hideMeasureOverlay(document);
  const btn = document.getElementById('btn-measure');
  btn?.classList.toggle('on', !!measureMode);
}

function toggleMeasureGrid() {
  measureMode = measureMode ? '' : 'window';
  applyMeasureGrid();
}

/**
 * 本文の枠(iframe)が、窓のどこに、どれだけの倍率で置かれているか。
 * 枠の中で測った値に倍率を掛けて左上の位置を足すと、窓の座標になる。
 * **固定レイアウトの本では枠が縮めてある**ので、倍率を掛けないと数が合わない。
 */
function contentOrigin(doc) {
  const el = doc?.defaultView?.frameElement;
  if (!el) return null;
  const r = el.getBoundingClientRect();
  return {
    x: r.left, y: r.top, w: r.width, h: r.height,
    scaleX: el.offsetWidth ? r.width / el.offsetWidth : 1,
    scaleY: el.offsetHeight ? r.height / el.offsetHeight : 1,
  };
}

/**
 * デバッグモードの入り切りをボタンへ反映する。
 * **切ったときは、出したままの物差しも消す。** ボタンが消えると消す手段が無くなるため。
 */
function applyDebugMode() {
  const on = !!settings?.debugMode;
  const btn = document.getElementById('btn-measure');
...
// 窓の大きさが変われば物差しも引き直す(出ているときだけ)。
window.addEventListener('resize', () => { if (measureMode) applyMeasureGrid(); });
...
/**
 * 辞書の画面を押したあと、**保存が終わるまで待つ**。
 *
 * ボタンを押す処理(click)はすぐ返るが、その中の保存は後から走る。待たずに次の命令を
 * 送ると、保存が古い中身で上書きして、入れたはずの登録が消える(実際に踏んだ)。
 *
 * @param {?string} appear 待つ目印。この選択子の要素が出たら終わり。
 *                         null なら、画面(モーダル)そのものが消えたら終わり。
 */
async function dictSettled(appear) {
  const done = () => (appear ? !!document.querySelector(appear) : !document.querySelector('.modal-backdrop'));
...
function registerReaderTestbus() {
  ...
  registerTestbus({
    ...
    // 縦書きが破綻していないかを、数で確かめるための値をまとめて返す。
    //
    // **判定はここでしない。数を返すだけにする。** 何をもって破綻とするかは
    // docs/internal の計画に書いてあり、engine ごとに少しずつ違う。
    // ここで判定まで書くと、基準を変えるたびにアプリを作り直すことになる。
    //
    // 見るのは 4 つ:
    //   1. 縦書きの宣言と、実際に組まれた向きが合っているか（htmlWM / bodyWM / dir）
    //   2. 本文が入れ物からはみ出していないか（overflowX / overflowY）
    //   3. ページを送ったとき、どの段落も一度は画面に出るか（paraInViewIdx を集める）
    //   4. ルビが画面の外へ出ていないか（rtOutside / rtTotal）
    //
    // **スクリーンショットの画素を比べる検査は作らない**（維持できない）。
    layoutInfo: async () => {
      const doc = cur()?.doc;
      ...
      const vw0 = win.innerWidth;
      const vh0 = win.innerHeight;
      const paras = [...doc.querySelectorAll('p')];
      // 「組まれている」段落。幅と高さがあれば、画面の外にあっても数える。
      const paraRendered = paras.filter((el) => {
        const r = el.getBoundingClientRect();
        return r.width > 0 && r.height > 0;
      }).length;
      // **いま画面の中に入っている**段落の番号。
      // ページを送りながらこれを集めれば、どの段落も一度は画面に出たかが分かる。
      // `paraRendered` はページを送っても変わらないので、欠落の判定には使えない。
      const paraInViewIdx = [];
      paras.forEach((el, i) => {
        const r = el.getBoundingClientRect();
        if (r.width <= 0 || r.height <= 0) return;
        if (r.right > 0 && r.left < vw0 && r.bottom > 0 && r.top < vh0) paraInViewIdx.push(i);
      });

      const rts = [...doc.querySelectorAll('rt')];
      const vw = vw0;
      const vh = vh0;
      // 1 画素の食い違いは丸めの差なので数えない。
      const margin = 1;
      const rtOutside = rts.filter((el) => {
        const r = el.getBoundingClientRect();
        return r.left < -margin || r.top < -margin || r.right > vw + margin || r.bottom > vh + margin;
      }).length;

      return {
        htmlWM: cs(html).writingMode,
        bodyWM: cs(body).writingMode,
        dir: html.getAttribute('dir') || cs(html).direction,
        ...
        paraRendered,
        paraInView: paraInViewIdx.length,
        paraInViewIdx,
        rtTotal: rts.length,
        rtOutside,
        rtDisplay: rts[0] ? cs(rts[0]).display : null,
        rtFontSize: rts[0] ? cs(rts[0]).fontSize : null,
        bodyFontSize: cs(body).fontSize,
        // 文字サイズの設定は `html { font-size: N% }` に効く。**body だけを見ても分からない。**
        // 本の CSS が body に絶対値（16px 等）を書いていると、html の % は本文に届かない。
        htmlFontSize: cs(html).fontSize,
        // 本文に指定されている書体（実際に使われた書体ではなく、CSS の指定そのもの）。
        bodyFontFamily: cs(body).fontFamily,
        // アプリの画面（本文の外側）の書体の指定。
        uiFontFamily: getComputedStyle(document.body).fontFamily,
        // **指定した書体が、この機械に本当にあるか。** `fontFamily` は指定を返すだけで、
        // 実際に使われた書体は返さない。
        //
        // **`document.fonts.check()` は使わない。** 無い書体でも true を返す
        // （Windows で 9 個ぜんぶ「ある」と返した。ヒラギノが入っていないのに）。
        // 代わりに、同じ文字を「その書体 → 等幅」と「等幅だけ」で描いて幅を比べる。
        // 幅が違えば、その書体が実際に使われたということである。
        fontsAvailable: (() => {
          try {
            const cx = document.createElement('canvas').getContext('2d');
            const probe = 'あいうえおMWmw漢字';
            ...
            return [
              ...
              'Yu Gothic UI', 'Yu Gothic', 'Meiryo', 'MS Mincho',
            ].map((name) => {
              cx.font = `72px "${name}", monospace`;
              return { name, ok: cx.measureText(probe).width !== base };
            });
          } catch { return null; }
        })(),
        // ページ送りの測り方を直すための材料（W6-5）。
        // **本文の側から見ると、どのページでも全段落が「画面の中」に入ってしまう。**
        // 段組みで横に並べたものを、外側の要素がずらして見せているためである。
        // ここでは、その仕組みが実際にどうなっているかを数で出す。
        viewportW: vw0,
        viewportH: vh0,
        // 最初の 5 段落が、いまどこにあるか（本文の座標系）。
        paraRects: paras.slice(0, 5).map((el) => {
          const r = el.getBoundingClientRect();
          return { l: Math.round(r.left), t: Math.round(r.top), w: Math.round(r.width), h: Math.round(r.height) };
        }),
        // 最後の 3 段落が、いまどこにあるか。
        paraRectsTail: paras.slice(-3).map((el) => {
          const r = el.getBoundingClientRect();
          return { l: Math.round(r.left), t: Math.round(r.top), w: Math.round(r.width), h: Math.round(r.height) };
        }),
        // **画面の中央にある要素。** 空白ページなら本文の要素が取れないはずである。
        atCenter: (() => {
          try {
            const el = doc.elementFromPoint(Math.round(vw0 / 2), Math.round(vh0 / 2));
            if (!el) return null;
            return { tag: el.tagName.toLowerCase(), text: (el.textContent || '').trim().slice(0, 30) };
          } catch { return null; }
        })(),
        // 縦中横と圏点も、使われていれば数える。
        tcuTotal: [...doc.querySelectorAll('*')].filter((el) => cs(el).textCombineUpright === 'all').length,
        emphasisTotal: [...doc.querySelectorAll('*')].filter((el) => {
          const v = cs(el).textEmphasisStyle;
          return v && v !== 'none';
        }).length,
      };
    },
    // 本文へ CSS を入れる 4 通りの方法を、その場で試して結果を返す。
    //
    // **なぜ要るか:** 文字サイズの設定が Windows で効かない。設定は foliate-js の
    // `renderer.setStyles()` に渡され、その中は `<style>` 要素の中身を書き換えている。
    // **どの入れ方なら通るのかが分かれば、直し方が決まる。**
    // testbus の `csp` と `setStyle` は外側の画面を見る命令で、本文の中は調べられない。
    //
    // 入れた指定は毎回すぐ取り除くので、画面は元のままになる。
    probeStyle: async () => {
      const doc = cur()?.doc;
      if (!doc) return { error: 'no document' };
      const win = doc.defaultView;
      const html = doc.documentElement;
      const size = () => win.getComputedStyle(html).fontSize;
      const before = size();
      const out = { before };

      // (1) <style> 要素を新しく作って入れる（foliate-js と同じやり方）
      try {
        ...
        out.byStyleTag = size();
        st.remove();
      } catch (e) { out.byStyleTag = 'エラー: ' + e.message; }

      // (2) 要素に直に指定を入れる
      try {
        html.style.setProperty('font-size', '150%', 'important');
        out.byInline = size();
        html.style.removeProperty('font-size');
      } catch (e) { out.byInline = 'エラー: ' + e.message; }

      // (3) adoptedStyleSheets で入れる（過去に macOS で同じ問題を直したときの方法）
      try {
        const sheet = new win.CSSStyleSheet();
        sheet.replaceSync('html { font-size: 175% !important; }');
        doc.adoptedStyleSheets = [...doc.adoptedStyleSheets, sheet];
        out.byAdopted = size();
        doc.adoptedStyleSheets = doc.adoptedStyleSheets.filter((x) => x !== sheet);
      } catch (e) { out.byAdopted = 'エラー: ' + e.message; }

      // (4) foliate-js が使っている <style> は本当にあるか。中身は入っているか。
      out.styleTags = [...doc.querySelectorAll('style')].map((st) => ({
        len: (st.textContent || '').length,
        head: (st.textContent || '').replace(/\s+/g, ' ').slice(0, 70),
      }));

      // (5) 本文の中に CSP の指定が入っていないか
      const meta = doc.querySelector('meta[http-equiv="Content-Security-Policy"]');
      out.innerCsp = meta ? meta.getAttribute('content') : null;
    ...
    setSetting: async ({ key, value }) => { settings[key] = value; await saveSettings(settings); applyReaderStyles(); applyDebugMode(); return { [key]: settings[key] }; },
    // M2 しおり
    ...
    // 測定オーバーレイ(§14.1–14.4)。物差しを出していなくても数は返る。
    measureState: async () => ({
      debugMode: !!settings.debugMode,
      buttonShown: document.getElementById('btn-measure')?.style.display !== 'none',
      shown: measure.isMeasureOverlayShown(document),
    }),
    measureToggle: async () => { toggleMeasureGrid(); return { shown: measure.isMeasureOverlayShown(document) }; },
    // 物差しに描いた色を読み戻して、band() の数と合っているかを見る。
    // 人が写真を見て色から座標を読むのと同じことを、機械で行う。
    measureProbe: async ({ x = null, y = null } = {}) => measure.readRibbon(document, { x, y }),
    // 本文の位置を測って返す(§14.4)。物差しを出していなくても数は返る。
    // 座標は窓の物差しと同じ枠にそろえてある。frame は本文の枠が窓のどこに在るか。
    measureOverlay: async () => {
      const doc = contentDocs()[0];
      if (!doc) return { error: 'no content' };
      const frame = contentOrigin(doc);
      const out = measure.measureDocument(doc, doc.defaultView || window, {
        origin: frame || { x: 0, y: 0 },
        scale: frame ? { x: frame.scaleX, y: frame.scaleY } : { x: 1, y: 1 },
        viewport: measure.visibleViewport(window),
      });
      out.frame = frame
        ? {
          x: measure.r1(frame.x), y: measure.r1(frame.y),
          w: measure.r1(frame.w), h: measure.r1(frame.h),
          scaleX: Math.round(frame.scaleX * 1000) / 1000,
          scaleY: Math.round(frame.scaleY * 1000) / 1000,
        }
        : null;
      return out;
    },
    ...
    highlightedText: async () => {
      // いま読み上げている文。帯は本文の上に重ねた矩形なので、範囲は tts が持っている。
      return { text: tts?.highlightedText?.() || '' };
    },
    ...
    // ---- 読み辞書 ----
    //
    // 命令の並びは Swift 版(spike-readium/Sources/TestBus.swift)にそろえてある。
    // 1 件ずつ足す・直す・消すを持たせているのは、速いからではなく、
    // 人が画面でやる操作と同じ形にするため。外から動かしても、
    // 画面が 1 件ずつ変わっていくのを利用者が目で追える。
    //
    // Swift 版と 1 つだけ違う点: 項目の身元を UUID ではなく「種類 + 表記」で決めている。
    // UUID は SwiftUI が一覧を描くために要求するもので、この版には要らない。
    // 表記を身元にすると、同じ表記が二重に登録されることが原理的に起きない
    // (Swift 版は id が違えば同じ表記を 2 つ持ててしまい、どちらが先に適用されるか決まらない)。
    // その代わり、表記そのものを書き換えるときは newSurface で指定する。

    // scope: 'common'(すべての本。既定) / 'book'(この本だけ) / 'merged'(読み上げに渡すのと同じ並び)
    dictList: async ({ scope = 'common' } = {}) => {
      // normalizeList が適用順(レイヤーの大きい順 → 表記の長い順)に並べ替えて返す。
      const { normalizeList } = await import('./dictionary.js');
      const list = scope === 'merged' ? await mergedDict() : await loadDictScoped(scope);
      return { scope, words: normalizeList(list) };
    },

    dictAdd: async ({ scope = 'common', ...entry } = {}) => {
      const { normalizeEntry, normalizeList } = await import('./dictionary.js');
      const e = normalizeEntry(entry);
      if (!e) return { ok: false, error: '表記と読みの両方が要る' };
      const list = normalizeList([...(await loadDictScoped(scope)), e]); // 同じ表記があれば後勝ちで 1 つになる
      return { ok: true, scope, count: (await saveDictList(list, scope)).length };
    },

    dictUpdate: async ({ surface, kind = 'word', newSurface, scope = 'common', ...rest }) => {
      const { normalizeEntry, normalizeList } = await import('./dictionary.js');
      const list = normalizeList(await loadDictScoped(scope));
      const i = list.findIndex((e) => e.surface === surface && e.kind === kind);
      if (i < 0) return { ok: false, error: '見つからない表記: ' + surface };
      const merged = normalizeEntry({ ...list[i], ...rest, surface: newSurface || surface, kind });
      if (!merged) return { ok: false, error: '直したあとの内容が空になる' };
      list[i] = merged;
      return { ok: true, scope, count: (await saveDictList(list, scope)).length };
    },

    dictDelete: async ({ surface, kind = 'word', scope = 'common' }) => {
      const { normalizeList } = await import('./dictionary.js');
      const list = normalizeList(await loadDictScoped(scope));
      const rest = list.filter((e) => !(e.surface === surface && e.kind === kind));
      if (rest.length === list.length) return { ok: false, error: '見つからない表記: ' + surface };
      return { ok: true, scope, count: (await saveDictList(rest, scope)).length };
    },

    // 丸ごと入れ替える。一括で流し込むときはこちら。
    setRules: async ({ entries, rules, scope = 'common' }) => {
      const { normalizeList } = await import('./dictionary.js');
      const list = normalizeList(entries || rules || []);
      return { ok: true, scope, count: (await saveDictList(list, scope)).length };
    },

    // 辞書を text へ適用した結果を返す。読み上げエンジンを起動せずに検算できる。
    // 適用は実際の読み上げと同じ並び(すべての本 → この本だけ)で試す。
    // place に { section, offset } を渡すと「この場所だけ」の登録も当たる。
    applyRules: async ({ text, place = null }) => {
      const { prepare } = await import('./dictionary.js');
      const r = prepare(text, await mergedDict(), place);
      // injectedGaps は Swift 版と同じ名前。silenceGaps はこの版が元から使っている名前。
      return { text: r.text, injectedGaps: r.silenceGaps, silenceGaps: r.silenceGaps, gapCount: r.gapCount, changed: r.changed };
    },

    // 右クリック登録と同じ経路で単語の登録画面を開く。
    // データだけ書き換えるのではなく、人が使う画面をそのまま出せるようにしておく。
    // place に { section, offset } を渡すと、本文を選んで「この場所だけの読みを登録」と
    // 押したときと同じ画面になる。登録済みの一覧はこの画面には出ない。
    openDictForm: async ({ surface = '', place = null } = {}) => {
      openDict(() => settings, {
        addSurface: String(surface).slice(0, 40), addPlace: place, view: 'entry',
        onClose: refreshDict, book: dictBook(),
      });
      return { ok: true, surface, place };
    },

    // 登録済みの一覧の画面を開く(登録画面とは別の画面)。
    openDictList: async () => {
      openDict(() => settings, { onClose: refreshDict, book: dictBook() });
      return { ok: true };
    },

    // 辞書の一覧から 1 件の編集画面へ進む(人が行を押すのと同じ)。
    dictOpenEntry: async ({ scope = 'common', index = 0 } = {}) => {
      const row = document.querySelector(`.dict-row[data-scope="${scope}"][data-i="${index}"]`);
      if (!row) return { ok: false, error: '行が見つからない: ' + scope + '/' + index };
      row.click();
      return { ok: true, scope, index };
    },

    // 編集画面の「適用範囲」を押して、その 1 件を反対側へ移す。
    dictSetScope: async ({ to = 'book' } = {}) => {
      const b = document.querySelector(`#de-scope button[data-scope="${to}"]`);
      if (!b) return { ok: false, error: '適用範囲の切り替えが画面に無い' };
      b.click();
      return { ok: true, to };
    },

    // 登録・編集の画面から一覧へ移る(保存される)。
    // 一覧から来たときは左上の「‹」、登録だけしに来たときは下の「登録済みの一覧を見る」。
    dictBack: async () => {
      const b = document.querySelector('#de-back') || document.querySelector('#de-open-list');
      if (!b) return { ok: false, error: '登録・編集の画面ではない' };
      b.click();
      return { ok: await dictSettled('#dict-close') };
    },

    // 登録の画面を保存して閉じる(「完了」を押すのと同じ)。
    dictDone: async () => {
      const b = document.querySelector('#de-done');
      if (!b) return { ok: false, error: '登録の画面ではない' };
      b.click();
      return { ok: await dictSettled(null) };
    },
    // 一覧の画面を閉じる(「閉じる」を押すのと同じ)。
    dictClose: async () => {
      const b = document.querySelector('#dict-close');
      if (!b) return { ok: false, error: '一覧の画面ではない' };
      b.click();
      return { ok: await dictSettled(null) };
    },

    // 以前からある名前。すでに書いた道具が動かなくなるので残してある。
    dictGet: async ({ scope = 'common' } = {}) => await loadDictScoped(scope),
    dictSet: async ({ entries, scope = 'common' }) => await saveDictList((await import('./dictionary.js')).normalizeList(entries || []), scope),
    dictPrepare: async ({ text, place = null }) => {
      const { prepare } = await import('./dictionary.js');
      const r = prepare(text, await mergedDict(), place);
    ...
    // 診断用: 現在文の帯の矩形(ルビ込み / ルビの読みを除いたもの)。
    // ルビのある行だけ帯が太くなる件を数値で見るために使う。
    ttsHighlightRects: async () => tts?.highlightRects?.() || null,
    ttsHighlightInfo: async () => {
      // 診断用: 現在文(tts)と読み終わった部分(tts-progress)の範囲テキスト(先頭20字)。
      // 読み終わった部分の塗り残りなどの検証に使う。
      // place は、その文が本のどこにあるか(「この場所だけ」の登録の照合先)。
      return { ...(tts?.highlightInfo?.() || {}), place: tts?.highlightPlace?.() || null };
    },
...
function onLoad({ detail }) {
  ...
  // 新しく開いた本文にも、文字サイズ等の指定を入れる（adoptReaderCSS のコメントを参照）。
  adoptReaderCSS(doc);
  ...
  // 本文で右クリック → 「しおりを追加」「読み上げ辞書に登録」のメニュー(§9.7)
  try {
    doc.addEventListener('contextmenu', onContentContextMenu);
    // 本文は iframe の中にあるので、外側の文書に付けた closePopup が届かない。
    // 本文を押したときにもメニューが閉じるよう、こちらにも付けておく。
...
/**
 * 選んだ範囲にしおりを挟む(Swift 版 ReaderView.swift:1997-2011 と同じ)。
 * 現在位置に挟む addBookmark() と違い、位置は選択範囲から作り、
 * 一覧に残す抜粋も**選んだ文そのもの**にする。現在位置のしおりはページの
 * 冒頭を抜粋にするので、どこに挟んだのか一覧で区別が付かなかった。
 */
async function addBookmarkAtSelection(text, range) {
  let cfi = null;
  try {
    const c = view.renderer.getContents()[0];
    if (c && range) cfi = view.getCFI(c.index, range);
  } catch { /* 範囲から位置を作れないときは下の行で現在位置に戻す */ }
  cfi = cfi || lastCfi;
  if (!cfi) return;
  const list = await loadBookmarks();
  if (list.some((b) => b.cfi === cfi)) return;      // 重複防止(現在位置の方と同じ扱い)
  const excerpt = text.replace(/\s+/g, ' ').trim().slice(0, 60);
  list.push({ cfi, fraction: lastFraction, excerpt, createdAt: Date.now() });
  await saveBookmarks(list);
  await renderBookmarks();
}
...
async function renderBookmarks() {
  ...
  if (!list.length) { box.innerHTML = `<p class="bm-empty">${t('reader.bmEmpty')}</p>`; return; }
  box.innerHTML = '';
...
function openCssEditor() {
  ...
  back.innerHTML = `<div class="modal">
    ...
    <div class="row stack">
      <label>${t('reader.cssCommon')}</label>
      <textarea id="css-common" class="css-box"></textarea></div>
    <div class="row stack">
      <label>${t('reader.cssBook')}</label>
      <textarea id="css-book" class="css-box"></textarea></div>
...
// ---------------------------------------------------------------------------
// 本文の右クリック → 「しおりを追加」「読み上げ辞書に登録」のメニュー(§9.7)
//
// Swift 版(ReaderView.swift:726-741)と同じ形にそろえてある。項目は上から
// 「しおりを追加」「読み上げ辞書に登録」の 2 つで、選択が空のときは出さずに
// 既定のメニューへ任せる。
//
// 以前はメニューを出さず、いきなり辞書の登録画面を開いていた。目的は果たせるが、
// 人が「しおりにするか、辞書に登録するか」を選ぶ場面がまるごと無かった。
// この試作版を仕様として作り直す方針から外れるので、メニューを出す形へ戻した。
// ---------------------------------------------------------------------------
function onContentContextMenu(e) {
  ...
  // 右クリックは本文の iframe の中で起きる。#ctxmenu は外側の文書にあるので、
  // iframe の位置を足して外側の座標へ直す。足さないとメニューがずれた所に出る。
  const fr = win.frameElement?.getBoundingClientRect();
  const at = { x: e.clientX + (fr?.left || 0), y: e.clientY + (fr?.top || 0) };

  // 選択範囲は、この後の非同期処理の間に消えることがある。先に控えておく。
  const range = sel.rangeCount ? sel.getRangeAt(0).cloneRange() : null;

  openPopup(at, [
    { label: t('reader.ctxBookmark'), run: () => addBookmarkAtSelection(text, range) },
    { label: t('reader.ctxDict'),
      run: () => openDict(() => settings, { addSurface: text.slice(0, 40), onClose: refreshDict, book: dictBook() }) },
    { label: t('reader.ctxDictHere'),
      run: () => openDict(() => settings, {
        addSurface: text.slice(0, 40), addPlace: placeOfRange(range),
        onClose: refreshDict, book: dictBook(),
      }) },
  ]);
}
...
/**
 * 選んだ範囲が本のどこにあるかを返す。{ section: 章の番号, offset: 章の先頭から数えた文字数 }。
 * 「この場所だけ」の読みを登録するときに使う。
 */
function placeOfRange(range) {
...
function onTTSError(code) {
  ...
  else if (code === 'ENGINE_LAUNCH_FAILED') toast(t('tts.engineLaunchFailed'));
}

/**
 * 読み上げエンジンの起動まわりの知らせ。
 * 起動を待っている間は出しっぱなしにする(十数秒かかることがあり、
 * 4 秒で消えると「押したのに何も起きない」と見えるため)。
 */
function onTTSNotice(code) {
  if (code === 'ENGINE_STARTING') toast(t('tts.engineStarting'), { persist: true });
  else if (code === 'ENGINE_READY') toast(t('tts.engineReady'));
}
 ...
 * anchor には要素のほかに {x, y} も渡せる。右クリックはマウスの位置に出すため
 * 基準になる要素が無く、座標で置く必要がある(Swift の右クリックメニューと同じ位置)。
 */
function openPopup(anchor, items) {
  ...
  menu.hidden = false;
  // 要素なら箱の大きさを測る。座標なら、その 1 点を高さ 0 の箱として扱う。
  const r = typeof anchor?.getBoundingClientRect === 'function'
    ? anchor.getBoundingClientRect()
    : { left: anchor.x, top: anchor.y, bottom: anchor.y };
...
function openReaderSettings() {
  openSettings(settings, async (s) => {
    ...
    applyReadingDirection();
    updateModeButtons();
    applyDebugMode();                       // 切ったら物差しも消す(消す手段が無くなるため)
...
function wireControls() {
  ...
  on('btn-measure', 'click', toggleMeasureGrid);
...
async function wireMenu() {
  await setupMenu('reader', {
    ...
    'tts.play': startTTS,
    'tts.stop': () => tts?.stop(),
    // 登録する画面(一覧を通らない)と、登録済みの一覧は別の項目にしてある。
    'tts.dictAdd': () => openDict(() => settings, { view: 'entry', onClose: refreshDict, book: dictBook() }),
    'tts.dict': () => openDict(() => settings, { onClose: refreshDict, book: dictBook() }),
...
main().catch((e) => {
  ...
  document.body.insertAdjacentHTML('beforeend',
    `<div class="load-error">読み込みに失敗しました<br><small>${e}</small></div>`);
});
