...
import { osFileName } from './typeset.js';
...
async function importFromPaths(paths, { announce = true } = {}) {
  ...
  for (const path of paths) {
    ...
    // Windows のパスは `\\` 区切りなので、`/` だけで切るとフルパスがそのまま名前になる。
    p.current = osFileName(path) || path;
    showImportBanner();
...
function heroCard() {
  ...
  e.innerHTML = `
    ...
    <div class="hero-info">
      ...
      ${missing ? `<div class="hero-progress t-warn">${escapeHtml(t('shelf.fileMissing'))}</div>` : ''}
    </div>`;
...
// 本 1 冊に紐づく保存キーの接頭辞。本を消すときはここを全部まとめて消す。
// 1 つでも残すと「消したはずの本の設定が、別の本に化けて出る」ことは無いものの、
// 誰にも使われないデータが溜まり続ける。本の id は取り込むたびに新しく振られるので、
// 同じ本を入れ直しても前のデータは戻らない = 残しておく意味が無い。
const BOOK_KEY_PREFIXES = [
  'dict-',    // この本だけの読み上げ辞書
  'css-',     // この本だけのカスタム CSS
  'bm-',      // しおり
  'loc-',     // 読書位置(CFI)
...
// 保存キーに使う id の形。reader.js の safeId()・store.js の bookKey()・
// prefs.js の keyOf() と同じ規則にそろえてある。ここがずれると消し残しが出る。
function bookKeyId(id) { return String(id || '').replace(/[^a-zA-Z0-9_-]/g, ''); }

async function removeBook(id) {
  await api.deleteBook(id);
  // EPUB 本体を消したあとに、その本に紐づく保存キーもまとめて消す。
  // 1 つ失敗しても残りは消すため、個別に握り潰す。
  const key = bookKeyId(id);
  await Promise.all(BOOK_KEY_PREFIXES.map(async (prefix) => {
    try { await api.storeDelete(prefix + key); } catch { /* 消せなくても続ける */ }
  }));
...
function openProfileManager() {
  ...
  back.innerHTML = `<div class="modal">
    <div class="sheet-bar">
      <span class="grow pm-title">${escapeHtml(t('shelf.profileManage'))}</span>
    ...
    <div class="modal-body">
      ...
      <div class="desc pm">${escapeHtml(t('shelf.profileDesc'))}</div>
      ...
      <div class="desc pm-flat">${escapeHtml(t('shelf.profileNewDesc'))}</div>
    </div>
  </div>`;
  ...
  const draw = () => {
    ...
    for (const p of profileIndex.profiles) {
      ...
      row.innerHTML = `<span class="sym pm-mark${cur ? ' on' : ''}">
        ...
        <span class="grow"><span class="pm-name"></span>${cur ? `<div class="pm-current">${escapeHtml(t('shelf.profileCurrent'))}</div>` : ''}</span>
        ...
        <button class="pm-remove danger"${canRemoveProfile(profileIndex, p.id) ? '' : ' disabled'}>${escapeHtml(t('shelf.profileRemove'))}</button>`;
...
async function main() {
  ...
  await setupMenu('shelf', {
    ...
    // 書棚には「この本」が無いので、どちらの画面でも「すべての本」だけを扱う。
    'tts.dictAdd': () => openDict(() => settings, { view: 'entry' }),
...
function registerShelfTestbus() {
  registerTestbus({
    ...
    // 読み上げ辞書の登録画面を開く。書棚には本が選ばれていないので、
    // 「この本だけ」は選べない(openDict に book を渡さないため)。
    openDictForm: async () => { openDict(() => settings, { view: 'entry' }); return { ok: true }; },
    // 登録済みの一覧の画面を開く(登録画面とは別の画面)。
    openDictList: async () => { openDict(() => settings); return { ok: true }; },
