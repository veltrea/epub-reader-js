...
(VOICEVOX / AivisSpeech). Runs on **macOS and Windows 11**. Supports both vertical
writing (right-to-left page turning) and horizontal writing.

Screens in this manual were captured on macOS. Windows shows the same items, with the
menu bar inside the window instead of at the top of the screen.
...
1. Install [VOICEVOX](https://voicevox.hiroshiba.jp/) (port `:50021`) or
...
**You do not have to start the engine yourself.** If it is not running when you press
read-aloud, the app starts it and waits until it can speak. Starting can take a dozen
seconds or so; meanwhile the bar at the bottom says "Starting the speech engine…".
Only VOICEVOX and AivisSpeech can be started this way. If you picked "Custom URL",
the app cannot tell which program to run, so start it yourself.
...
| Delete | "Delete" from the book's menu (with a confirmation dialog). **Its bookmarks, reading position, and its book-only dictionary and custom CSS are deleted with it** |
...
### Measuring grid (debug mode)

**This is a tool for fixing typesetting. It is not something you need in order to read.**

The Display tab of the settings sheet has a **Debug mode** checkbox, off by default.
Turning it on adds a **Measuring grid** button to the reading toolbar. Pressing it lays a
colour ruler over the screen; pressing it again takes the ruler away.

The ruler carries the horizontal coordinate along the top edge and the vertical
coordinate along the left edge, as **two lanes of sixteen colours**. Read the two cells at
an edge, multiply the upper lane by 16 and add the lower lane, and you have the cell
number. One cell is 10 pixels, so ten times the cell number is the coordinate. A legend
mapping the sixteen colours to 0–F sits in the middle of the screen. Grid lines every 10%,
an outer frame, a centre cross and the viewport size are drawn alongside it.

**A photograph of the screen is then enough to read off how wide a margin is, or how far
something overflows.** No ruler against the display, no estimating by eye.

Clearing the debug-mode checkbox also takes away a ruler that is still on screen.
...
| Add to reading dictionary… | Register one word ([section 5](#reading-dictionary-fixing-mis-readings)) | |
| Edit reading dictionary… | Review and edit what is saved ([section 5](#reading-dictionary-fixing-mis-readings)) | |
...
- **The action on expiry** is one of three. Read-aloud always stops, whichever you pick.
  ...
  - **Sleep this computer** — stop, then put the computer to sleep
  - **Shut down this computer** — stop, then shut the computer down
...
> **Note**: "Sleep this computer" and "Shut down this computer" affect **the whole machine**,
> not just this app. Don't use them with unsaved work in other applications. On macOS the
> shutdown goes through System Events, so it stops if another app refuses to quit.
...
Register a reading to fix proper nouns the engine gets wrong.

**Registering and browsing are two separate screens.** Adding one word no longer makes you
scroll past every entry you have ever saved.

| What you want | How to open it | What appears |
|---|---|---|
| Register one word | Select a word in the text, right-click, "Add to reading dictionary" | The registration screen |
| Register one word | Menu **Read aloud > Add to reading dictionary…** | The registration screen |
| Review and edit what is saved | Menu **Read aloud > Edit reading dictionary…** | The list screen |

**The registration screen shows only the text, the reading, and the options.** No list of
saved entries. "Done" saves and closes it. If you do want the list, **"Browse saved entries"**
at the bottom of the same screen takes you there.

**If you have registered that word before, the screen opens with the saved reading already
filled in.** It does not create a second empty entry — two entries with the same text would
leave you guessing which one is in effect. The heading then reads **"Edit a saved reading"**.
The search order matches read-aloud: "This book only" first, then "All books".

**The list screen is for reviewing and editing what is saved.** Tap a row to open that one
entry; "‹" at the top left returns to the list. The list also has a "Test" box that shows
what a sentence becomes.

**The dictionary has two halves.**

| | Where it applies | What it is for |
|---|---|---|
| All books | Every book on that shelf | General fixes, e.g. always read 明日 as アシタ |
| This book only | The book you have open | Character names and the like, which read differently in another book |

**You pick the half on the registration screen, under "Applies to".** "All books" and
"This book only" sit side by side; tap one. "All books" is the default. The same control
moves an existing entry to the other half later. On the shelf (no book open) there is no
"this book", so the control does not appear.

When the same text appears in both, **the "This book only" entry is the one used**. Delete
it and the "All books" reading comes back.

**Layers decide the order of replacement.** Higher layers are applied first, and what they
replaced is left alone by lower layers. Put a longer word on a higher layer and a one-character
entry cannot take it first (registering 斎 will not also change 斎藤).

#### A reading for one spot only

Sometimes only one occurrence should read differently. Select the word in the text,
right-click, and choose **"Add a reading for this spot only"**.

**The position is a hint; what gets replaced is decided by matching the text.** Character
counts differ between tools, so trusting a position outright would replace a spot one
character off. Instead the app looks for the *same text* near the given position and
replaces that. If the same word appears twice nearby, only the one **closer to the spot**
changes. The same word further away is left alone.

"How far to look" in the entry screen sets the window (characters before and after;
40 by default). "Remove the spot" turns it back into an ordinary entry for the whole book.

Entries tied to a spot cannot be tried in the "Test" field — there is no reading position
to match against.

#### Conditions on the surrounding characters (no regular expression needed)

**A single-kanji entry eats compounds.** Register 行 as オコナ and 銀行 and 旅行 change too.
The "Surrounding characters" fields prevent that — just pick from **"Preceded by"** and
**"Followed by"**.

| Choice | Meaning |
|---|---|
| anything | no condition (default) |
| hiragana | that character is hiragana |
| katakana | that character is katakana |
| a kanji | that character is a kanji |
| not a kanji | that character is **not** a kanji |
| a digit | that character is a digit |

Examples:

| Spelling | Preceded by | Result |
|---|---|---|
| 行 | not a kanji | 銀行と旅行、行った → compounds stay, only the standalone 行 changes |
| 行った | hiragana | とり行った → とりオコナッタ; 銀行った is left alone |
| 年 | a digit | 五年と5年 → only 5年 changes |

Set both and the entry applies only when **both** hold. Entries with different conditions
are kept separately, so 行 preceded by hiragana and 行 preceded by a kanji can both exist.

#### Keeping the preceding characters (patterns)

Choose "Pattern" instead of "Word" and the spelling field takes a regular expression.
**Anything in parentheses is available in the reading as `$1`, `$2`, …**

There are two ways to write it.

**1. Capture and put back.** Anything in parentheses lands in `$1`, `$2`, …

| Spelling | Reading | Result |
|---|---|---|
| `([ぁ-ん]+)行った` | `$1オコナッタ` | とり行った → トリオコナッタ |
| `([をがはに])行った` | `$1オコナッタ` | 会議を行った → 会議ヲオコナッタ; 学校へ行った is left alone |
| `([0-9]+)年` | `$1ネン` | 1927年 → 1927ネン |

Captured text is converted to katakana. `$&` is the whole match and `$$` is a literal `$`.

**2. Make the surroundings a condition, without replacing them.** Shorter to write.

| Spelling | Meaning | Result |
|---|---|---|
| `(?<=[ぁ-ん])行った` | only when preceded by hiragana | とり行った → とりオコナッタ; 銀行った untouched |
| `(?<![一-龥])行` | only when **not** preceded by a kanji | 銀行, 旅行 stay; the standalone 行 is replaced |
| `行(?=事)` | only when followed by 事 | 行事 is replaced, 行った is not |

`(?<=…)` is a condition on what comes before, `(?<!…)` that it is *not* that, and `(?=…)`
a condition on what follows. **This is how you stop a single-kanji entry from eating
compounds.**

Handy character ranges:
...
Type a sentence in "Test" and you get the result in the same order the read-aloud uses
(all books, then this book).

It only affects read-aloud; the text on screen never changes. The engine's own user
dictionary is not used — it cannot stop a short entry from swallowing a longer compound.

The "All books" dictionary is kept **per shelf**. The "This book only" one belongs to the book.
...
| Symptom | What to do |
|---|---|
| No sound when you press play | If the engine is not running, the app starts it and shows "Starting the speech engine…" at the bottom. If nothing happens after a dozen seconds, or you see "Could not start the speech engine", check that VOICEVOX or AivisSpeech is installed. With "Custom URL" the app does not start anything — start it yourself. If the address is wrong you get "Cannot reach the read-aloud engine" |
